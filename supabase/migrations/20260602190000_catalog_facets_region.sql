-- Make the faceted rail counts region-aware, matching the grid's per-country
-- streaming filter (#6). When a streaming service is selected we now match against
-- the `streaming_regions` column using "Provider:REGION" tokens built from the
-- viewer's account region (default US), instead of the US-only `streaming` column —
-- so genre/origin counts stay consistent with the grid for a non-US viewer.
--
-- Everything else is unchanged from 20260602140000_catalog_facets_filtered.

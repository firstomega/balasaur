-- Facets v4: the rail's headline rating filter is now the unified Balasaur
-- Score, so the facet counts must respect a balasaur_min/max/unrated predicate
-- (same "full range + include unrated = unconstrained" contract as the
-- per-source sliders) and report scored.balasaur coverage for the slider's
-- "N of M scored" line. Body otherwise identical to 20260811101000's version.
...
declare
  ...
  v_bala_min   numeric := coalesce((p->>'balasaur_min')::numeric, 0);
  v_bala_max   numeric := coalesce((p->>'balasaur_max')::numeric, 100);
  v_bala_un    boolean := coalesce((p->>'balasaur_unrated')::boolean, true);
...
begin
  with _base as materialized (
    select media_type, genres, origins, sub_genres, themes, audience,
           ...
           rating_imdb, rating_rotten_tomatoes, rating_metacritic, rating_balasaur
    from public.media
    where coalesce(sensitive, false) = false
      ...
      and (case when v_bala_un then (rating_balasaur is null or rating_balasaur between v_bala_min and v_bala_max)
                else (rating_balasaur is not null and rating_balasaur between v_bala_min and v_bala_max) end)
  ...
  select json_build_object(
    ...
    'scored', json_build_object(
      ...
      'balasaur', (select count(*) from _f where m_g and m_o and m_sg and m_th and m_au and m_cs and m_fl and rating_balasaur is not null)
    )
  ) into result;

-- Dynamic (faceted) counts for the filter rail. Given the active filters, returns
-- per-genre and per-origin counts that recompute as you narrow other filters —
-- standard faceted search, where a facet does NOT constrain its own counts (genre
-- counts apply every filter EXCEPT the genre selection; origin counts apply every
-- filter EXCEPT the origin selection). Read-only; safe to call on every filter change.
--
-- The predicate mirrors queryCatalog exactly (incl. year as TEXT compare, the
-- include-unrated rating logic, and "by person" jsonb containment) so the counts
-- match the grid.
...
declare
  ...
  result json;
begin
  -- `common`: every filter EXCEPT genre and origin. The genre/origin filters are
  -- then applied cheaply on top for each facet (and both for total/scored).
  create temp table _facet_common on commit drop as
  ...
  where (cardinality(v_types) = 0 or media_type = any (v_types))
    and (cardinality(v_streaming) = 0 or streaming && v_streaming)
    and (
  ...
  select json_build_object(
    -- genre counts: common + the ORIGIN filter (genre dimension excluded)
    'genres', (
    ...
    -- origin counts: common + the GENRE filter (origin dimension excluded)
    'origins', (
    ...
    -- total + scored: both genre and origin applied
    'total', (

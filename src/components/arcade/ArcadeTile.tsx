...
// texture, the mark large, the name in the display weight under it, a
// two-line hook slot, and a footer with the minutes pill and, after mount,
// either what you did today ("Solved in 3") or the streak you are
// protecting. The name sits at a fixed distance under the mark and the hook
// slot is a fixed two lines, so every tile in a row shares one baseline.
// A NEW ribbon comes only from the registry's isNewGame, computed after
// mount so cached HTML never disagrees with the clock. Hero tiles fan three
// real posters from yesterday's round, or a large faint mark when there is
// no round to show. The whole tile is the link.
...
// in the ground; the cool ones can afford to sit deeper.
...
function ground(hue: ArcadeHue): CSSProperties {
  ...
  const lo = warm ? 30 : 22;
  return {
    backgroundImage: `radial-gradient(rgba(255,255,255,0.09) 1px, transparent 1px), linear-gradient(165deg, color-mix(in oklch, var(--game, var(--primary)) ${hi}%, #0b0d10), color-mix(in oklch, var(--game, var(--primary)) ${lo}%, #0b0d10))`,
...
export function ArcadeTile({
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-[6px] border border-white/10 text-white transition-[transform,box-shadow] duration-200",
    ...
    >
      ...
      <div className={cn("relative min-w-0", hero ? "pt-5" : "pt-4", fan.length > 0 && "pr-28")}>
        <span
          className={cn(
            "block font-black leading-[1.02] tracking-[-0.02em] text-white",
            hero ? "text-[26px] sm:text-[30px]" : "text-[17px]",
          )}
        >
          {game.name}
        </span>
        <p
          className={cn(
            "mt-1.5 overflow-hidden text-white/85",
            hero ? "h-[41px] text-[14.5px] leading-[1.4]" : "h-[35px] text-[13px] leading-[1.35]",
          )}
        >
          {tileHook(game)}
        </p>

...
export function useUserStatus() {
  ...
  const recordStatus = useCallback<RecordStatusFn>(
    (id, record, item) => {
      ...
      //
      // IMPORTANT: a supabase query builder is a *lazy* thenable — the HTTP
      // request is only dispatched when the query is awaited or `.then()`-ed.
      // A bare `void supabase.from(...).upsert(...)` builds the query and
      // throws it away WITHOUT ever sending it, so the write silently never
      // happened (while the optimistic UI still flashed a success checkmark,
      // and the next reload found nothing). The trailing `.then()` is what
      // actually fires the request; it also surfaces any error rather than
      // swallowing it.
      if (user) {
        if (record === null) {
          void supabase
          ...
          void supabase
            ...
            .then(({ error }) => {
              if (error) console.error("[userStatus] save failed:", error.message);
            });
        }
      }
    },
